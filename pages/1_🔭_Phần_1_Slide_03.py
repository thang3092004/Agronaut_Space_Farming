...
show_understand_btn = stage < len(STAGED_TEXTS) - 1
show_stage_back_btn = stage > 0
...
col = st.columns([1,1])
with col[0]:
    back = st.button("⬅️ Back", key="_back_s3_main")
with col[1]:
    nxt = st.button("Continue ➡️", key="_next_s3_main")

if back:
    # Reset stage when leaving slide
    st.session_state.s3_1_understand_stage = 0
